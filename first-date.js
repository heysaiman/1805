 ...
 * ============================================================================
 * Architecture: Modular Portal Injection
...
document.addEventListener('DOMContentLoaded', () => {
    const portal = document.getElementById('first-date-portal');
    if (!portal) return;
    ...
    const AST = {
        ...
        // --- 2. FEED POST CAROUSEL (The Galleria & The Movie) ---
        post1_img1: "banner1.jpg", // Utkal Galleria Arrival
        post1_img2: "banner2.jpg", // The Mall Stroll
        post1_img3: "banner3.jpg", // Cinepolis Screen
        
        // --- 3. REELS ---
        reelVideo: "first_date_reel.mp4",
        
        // --- 4. EXPLORE GRID (9 Images) ---
        explore: [
    ...
    // ========================================================================
    // I am building out the entire SVG library within the string to ensure zero 
    // external dependencies and pixel-perfect rendering.
    portal.innerHTML = `
        <!-- MAIN APP CONTAINER -->
        <div id="ig-app-panel" class="fixed top-0 right-0 w-full md:w-[420px] h-[100dvh] bg-[#000000] z-[10000] transform translate-x-full transition-transform duration-700 ease-[cubic-bezier(0.16,1,0.3,1)] overflow-hidden flex flex-col border-l border-white/10 shadow-[0_0_80px_rgba(0,0,0,0.9)] text-white font-sans">
            ...
            <div id="ig-screens" class="flex-1 relative bg-[#000000]">
                ...
                <div id="screen-explore" class="absolute inset-0 overflow-y-auto no-scrollbar opacity-0 pointer-events-none transition-opacity duration-300 z-10 pb-20 bg-black">
                    ...
                    <!-- The Grid (Asymmetrical IG Layout) -->
                    <div class="grid grid-cols-3 gap-[2px]">
                        ...
                        <!-- Row 2: Contains the tall vertical span block -->
                ...
                <div id="screen-reels" class="absolute inset-0 overflow-hidden opacity-0 pointer-events-none transition-opacity duration-300 z-10 bg-black">
                    ...
                    <div class="relative w-full h-full bg-zinc-900" id="reel-container">
                        <!-- We use an image fallback placeholder if video file isn't uploaded yet -->
                        ...
                        <!-- Interactions Stack (Bottom Right) -->
                        <div class="absolute bottom-20 right-3 z-30 flex flex-col items-center gap-6">
                            ...
                            <div class="flex flex-col items-center gap-1.5 cursor-pointer">
                                <svg class="w-8 h-8 text-white drop-shadow-[0_2px_10px_rgba(0,0,0,0.5)] transition-transform active:scale-75" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M12 5v.01M12 12v.01M12 19v.01M12 6a1 1 0 110-2 1 1 0 010 2zm0 7a1 1 0 110-2 1 1 0 010 2zm0 7a1 1 0 110-2 1 1 0 010 2z"/></svg>
                            </div>
                ...
                <div class="relative z-10 flex-1 flex flex-col pt-4">
                    ...
                    <div class="flex-1 relative flex items-center justify-center p-0 mt-4 rounded-t-2xl overflow-hidden shadow-2xl">
                        ...
                        <div id="story-media-display" class="w-full h-full bg-cover bg-center transition-all duration-300" style="background-image: url('${AST.story1}');">
                            <!-- Temporary label if no image loads -->
            ...
            <!-- ========================================== -->
            <div id="comment-sheet" class="fixed inset-x-0 bottom-0 h-[70dvh] bg-[#1a1a1a] rounded-t-3xl z-[30000] flex flex-col sheet-closed border-t border-white/10 shadow-[0_-10px_40px_rgba(0,0,0,0.5)] md:w-[420px] md:right-0 md:left-auto">
    ...
    // 1. ROUTER EXPOSURE
    if (window.ModularRouter) {
        window.ModularRouter.openFirstDate = function() {
    ...
    if (carousel && dots && indicator) {
        carousel.addEventListener('scroll', () => {
            ...
            const activeIndex = Math.round(scrollPercent * 2); // 3 items = indexes 0, 1, 2
    ...
    const triggerLike = () => {
        ...
        // Trigger Giant Heart Overlay
        if (heartAnim) {
            heartAnim.classList.remove('heart-burst-anim');
            void heartAnim.offsetWidth; // Reflow
    ...
    // 6. TOUCH PHYSICS: COMMENT BOTTOM SHEET
    ...
    if (handle && sheet) {
        handle.addEventListener('touchstart', (e) => {
            ...
            isDragging = true;
            sheet.style.transition = 'none'; // Remove transition while dragging
        }, { passive: true });
        ...
        handle.addEventListener('touchend', () => {
            ...
            if (currentY > 150) {
                ...
                sheet.style.transform = 'translateY(0%)'; // Snap back up
            }
            currentY = 0;
        });
    }

    // 7. STATEFUL STORY VIEWER
    const storyData = [
    ...
    const loadStory = (idx) => {
        ...
        progressFills.forEach((fill, i) => {
            ...
            if (i < idx) {
                ...
                fill.style.width = '0%';
                fill.classList.add('story-progress-bar'); // Active
            } else {
                fill.style.width = '0%'; // Pending
            }
        });
    ...
    const resumeStory = () => {
        ...
        // Restart timer (approximation for demo)
