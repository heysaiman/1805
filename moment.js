...
// ✧----------------- GIRLFRIEND'S DAY HALF-MINUTE ANIMATION (AUG 1 & 2) -----------------✧
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (today.getMonth() === 7 && (today.getDate() === 1 || today.getDate() === 2)) {
        ...
        if (tiltCard && countdownContainer) {
            // Wait a brief moment to ensure the countdown has loaded, then fade it out
            setTimeout(() => {
                const originalDisplay = countdownContainer.style.display || '';
                countdownContainer.style.transition = 'opacity 0.6s ease';
                countdownContainer.style.opacity = '0';
                
                setTimeout(() => {
                    countdownContainer.style.display = 'none'; // Completely hide countdown
                    
                    // Create the beautiful animated message
                    ...
                    gfDayDiv.style.transition = 'opacity 0.8s ease-in-out';
                    gfDayDiv.innerHTML = `
                    ...
                    // Inject it inside the glass card
                    tiltCard.insertBefore(gfDayDiv, countdownContainer);
                    
                    // Trigger the fade-in for the message
                    setTimeout(() => {
                        gfDayDiv.style.opacity = '1';
                    }, 50);

                    // Half a minute (30 seconds) later, fade it out and restore the countdown
                    setTimeout(() => {
                        gfDayDiv.style.opacity = '0';
                        ...
                        setTimeout(() => {
                            ...
                            // Small delay to allow display to apply before fading the countdown back in
                            setTimeout(() => {
                                countdownContainer.style.opacity = '1';
                            }, 50);
                        }, 800); // Wait 0.8 seconds for the text fade-out to finish
                    }, 30000); // 30,000 milliseconds = 30 seconds
                    
                }, 600); // Wait for countdown to fade out first
            }, 100); 
        }
    }
});
