...
// ✧----------------- AUGUST 1ST GIRLFRIEND'S DAY ANIMATION -----------------✧
document.addEventListener('DOMContentLoaded', () => {
const today = new Date();

// Check if today is August 1st (Months are 0-indexed in JS, so 7 = August)
if (today.getMonth() === 7 && today.getDate() === 1) {
const tiltCard = document.getElementById('tilt-card');
const countdownContainer = document.getElementById('countdown-container');

if (tiltCard && countdownContainer) {
// 1. Temporarily hide the ticking countdown
const originalDisplay = countdownContainer.style.display || '';
countdownContainer.style.display = 'none';

// 2. Create the beautiful animated message
const gfDayDiv = document.createElement('div');
gfDayDiv.id = 'gf-day-overlay';
gfDayDiv.className = 'flex flex-col items-center justify-center text-center w-full py-8 reveal-element';
gfDayDiv.innerHTML = `
<h2 class="text-3xl md:text-5xl font-playfair font-black uppercase tracking-wider text-transparent bg-clip-text bg-gradient-to-r from-[#fcf3d7] to-[#d4af37] drop-shadow-[0_0_20px_rgba(212,175,55,0.6)] mb-6" style="animation: heartbeatPulse 1.5s ease-in-out infinite;">
Happy Girlfriend's Day
</h2>
<p class="text-gray-300 text-sm md:text-lg max-w-lg leading-relaxed font-light">
Wishing happy girlfriend's day to my capable and gorgeous partner... <br>
<span class="text-emerald-400 font-semibold italic tracking-wide mt-3 block">oh sorry, my third parent !</span>
</p>
`;

// 3. Inject it inside the glass card
tiltCard.appendChild(gfDayDiv);

// 4. Exactly one minute (60,000 ms) later, fade it out and restore the countdown
setTimeout(() => {
gfDayDiv.style.transition = 'opacity 1s ease-out';
...
countdownContainer.style.animation = 'luxury-reveal 1.5s cubic-bezier(0.16, 1, 0.3, 1) forwards';
